...
import { LOCAL_ACCOUNTS, LOCAL_SUPABASE_URL, loginAs } from '../fixtures/auth.mjs';
...
async function closeDialog(dialog) {
  const close = dialog.getByRole('button', { name: /Cancelar|Fechar|Concluir|Dispensar/ }).first();
  await close.click();
...
test('@a11y login e biblioteca não têm violações axe críticas ou sérias', async ({ page }) => {
  ...
  await expectAccessible(page, 'biblioteca');
});
...
test('@a11y diálogos da biblioteca não têm violações axe críticas ou sérias', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Sugerir edição' }).first().click();
  dialog = page.getByRole('dialog', { name: /Sugerir edição/ });
  await expectAccessible(page, 'sugerir edição', DIALOG);
  await closeDialog(dialog);

  await page.getByRole('button', { name: 'Solicitar arquivamento' }).first().click();
